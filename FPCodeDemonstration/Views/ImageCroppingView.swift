...
var windowScene = scenes.first as? UIWindowScene

var UniversalSafeOffsets = windowScene?.windows.first?.safeAreaInsets

struct ImageCroppingView: View {
    //These are the initial dimension of the actual image
    @State var imageWidth:CGFloat = 0
    @State var imageHeight:CGFloat = 0
    @State var rotationNum: CGFloat = 0

    @Binding var shown:Bool

    @State var croppingOffset = CGSize(width: 0, height: 0)
    @State var croppingMagnification:CGFloat = 0
                ...
                HStack {
                    ...
                    Button(action: {
                        ...
                        if let cImage = cgImage.cropping(to: CGRect(x: xOffset, y: yOffset, width: scaledDim, height: scaledDim)){
                            croppedImage = Image(uiImage: UIImage(cgImage: cImage))
                            shown = false
                        }
  ...
  var confirmBtn: some View {
    ZStack {
      ...
      Text("Confirm")
        .foregroundColor(.FPView)
    }

    .frame(width: 93, height: 37)
  }
}
